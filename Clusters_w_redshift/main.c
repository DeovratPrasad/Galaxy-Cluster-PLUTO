...
*/
  ...
  /* ------------------------------------------------------
     ...
     ------------------------------------------------------ */
    double z_prev=g_zred, Mh_prev=MhaloMcb(g_inputParam[M200],g_zred);
    g_time += g_dt;
// also update redshift, cosmic time, outer den/prs etc..
    g_tcosmic = g_time + Redtotcosmic(g_inputParam[zbeg])/(UNIT_LENGTH/UNIT_VELOCITY); //t=0 for code corresponds to zbeg
    g_zred = Tcosmictored(g_tcosmic*UNIT_LENGTH/UNIT_VELOCITY); 
    double Ezrat=Hubatz(g_zred)/Hubatz(z_prev);
    double M200_z=MhaloMcb(g_inputParam[M200],g_zred), R200_z=M200toR200(M200_z,g_zred);
    double M200_zp=MhaloMcb(g_inputParam[M200],z_prev), R200_zp=M200toR200(M200_zp,z_prev);

    g_d_outer = pow(R200_z/R200_zp,-g_inputParam[dslope])*Ezrat*Ezrat*g_d_outer;

    g_p_outer = pow(R200_z/R200_zp,-g_inputParam[dslope])*pow(Ezrat,8./3.)*pow(M200_z/M200_zp,2./3.)*g_p_outer;

//    print ("%20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e\n", g_time, g_tcosmic, M200_z/M200_zp, R200_z/R200_zp, Ezrat, g_p_outer, g_d_outer);
  /* ------------------------------------------------------
